    ...
    def _next(self, x: ndarray, func_callback, grad_func_callback) -> ndarray:
        ...
        if isinstance(self.alpha_optim, Optim):
            ...
            self.alpha = self.alpha_optim.optimize(
                func_callback=lambda alpha: func_callback(
                    x - alpha * grad_func_callback(x)
                ),
                lower_bound=0,
                upper_bound=1,
            )
...
class NesterovAcceleratedGradientDescent(Optim):
    ...
    def _next(self, x: ndarray, grad_func_callback) -> ndarray:
        ...
        self.momentum = (
            self.momemtum_coff * self.momentum
...
class Adam(Optim):
    ...
    def __init__(
        self, alpha: float = 0.01, beta_1: float = 0.9, beta_2: float = 0.99
    ) -> None:
        self.alpha = alpha
        self.beta_1 = beta_1
        self.beta_2 = beta_2
